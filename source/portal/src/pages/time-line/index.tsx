...
const TimelineContent: React.FC = () => {
  ...
  useEffect(() => {
    fetch(
      'https://api.github.com/repos/awslabs/sensitive-data-protection-on-aws/releases'
    )
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
  ...
  return (
    <ContentLayout>
      {(gitData || []).map((item, index) => {
        ...
        // publishTime = '';
        // body = item['body'];
        const publishTime = item['created_at'] as string;
        ...
        // const endStr =
        //   contributorsEndStr > -1 ? contributorsEndStr : fullChangeLogEndStr;
        return (
          //   <Container
                  ...
                  <Header
                    ...
                    description={
                      <div style={{ color: 'grey' }}>
                        {t('timeline:publishTime')}{' '}
                        {publishTime.replace('T', ' ').replace('Z', '')}
                      </div>
                    }
                    actions={
                      <Button
